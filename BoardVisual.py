...
class BoardVisual:
    ...
    def initialize(self):
        for col in range(1, 16):
            label = Label(self.boardframe, text=str(col))
            label.grid(row=0, column=col)
        for row in range(1, 16):
            label = Label(self.boardframe, text=str(row))
        ...
        for row in range(1, 16):
            for col in range(1, 16):
                button = Button(self.boardframe, height=3, width=3, bg="bisque", fg="gray1", text=" ")
